...
function getThreadMessages(id) {
  return messages
    .filter((message) => threadId(message) === id)
...
function filteredMessages() {
  ...
  const receivedThreadIds = new Set(messages
    .filter((message) => String(messageValue(message, 'Recipient', 'recipient')).toLowerCase() === username)
    .map((message) => threadId(message)));
  const visibleMessages = messages
    .filter((message) => {
      const sender = String(messageValue(message, 'Sender', 'sender')).toLowerCase();
      const recipient = String(messageValue(message, 'Recipient', 'recipient')).toLowerCase();
      return activeView === 'inbox'
        ? recipient === username || (sender === username && receivedThreadIds.has(threadId(message)))
        : sender === username;
    })
...
function renderList() {
  ...
  conversationList.innerHTML = rows.map((message) => {
    ...
    const recipient = messageValue(message, 'Recipient Name', 'recipientName') || messageValue(message, 'Recipient', 'recipient');
    const person = activeView === 'inbox' ? sender : `To ${recipient}`;
    const subject = messageValue(message, 'Subject', 'subject') || '(No subject)';
    const unread = thread.some((item) => String(messageValue(item, 'Recipient', 'recipient')).toLowerCase() === currentUsername().toLowerCase() && !isRead(item));
...
function renderViewer(message) {
  ...
  const replyTarget = [...thread].reverse().find((item) => String(messageValue(item, 'Sender', 'sender')).toLowerCase() !== currentUsername().toLowerCase()) || thread[0];
  const messageCards = thread.map((item, index) => {
    ...
    const recipient = messageValue(item, 'Recipient Name', 'recipientName') || messageValue(item, 'Recipient', 'recipient');
    let attachments = [];
  ...
  messageViewer.innerHTML = `<div class="message-detail"><div class="message-detail-top"><div><div class="eyebrow">Conversation</div><h2>${escapeMessageHtml(subject)}</h2></div><button class="action-btn reply-message-btn" type="button" data-reply-message-id="${escapeMessageHtml(messageId(replyTarget))}">Reply</button></div><div class="thread-messages">${messageCards}</div><form class="inline-reply-form" data-inline-reply-id="${escapeMessageHtml(messageId(replyTarget))}" hidden><label for="inlineReplyBody">Reply to ${escapeMessageHtml(messageValue(replyTarget, 'Sender Name', 'senderName') || messageValue(replyTarget, 'Sender', 'sender'))}</label><div class="format-toolbar" role="toolbar" aria-label="Text formatting"><button type="button" data-format-command="bold" aria-label="Bold"><strong>B</strong></button><button type="button" data-format-command="italic" aria-label="Italic"><em>I</em></button><button type="button" data-format-command="underline" aria-label="Underline"><u>U</u></button><button type="button" data-format-command="insertUnorderedList" aria-label="Bulleted list">•</button><button type="button" data-format-command="insertOrderedList" aria-label="Numbered list">1.</button><button type="button" data-format-command="removeFormat" aria-label="Clear formatting">Tx</button></div><div id="inlineReplyBody" class="rich-text-editor" contenteditable="true" role="textbox" aria-multiline="true" data-placeholder="Write your reply…"></div><label class="attachment-button" for="inlineReplyAttachments">Attach files</label><input id="inlineReplyAttachments" type="file" multiple hidden /><div class="attachment-list" id="inlineReplyAttachmentList"></div><div class="inline-reply-actions"><button class="action-btn" type="button" data-cancel-reply>Cancel</button><button class="action-btn primary" type="submit">Send reply</button></div></form></div>`;
...
async function selectMessage(id) {
  ...
  const unreadThreadMessages = getThreadMessages(threadId(message)).filter((item) => {
    return String(messageValue(item, 'Recipient', 'recipient')).toLowerCase() === currentUser && !isRead(item);
  });
...
function updateUnreadCount() {
  const unread = messages.filter((message) => String(messageValue(message, 'Recipient', 'recipient')).toLowerCase() === currentUsername().toLowerCase() && !isRead(message)).length;
...
async function loadMessages() {
  try {
    ...
    if (first) await selectMessage(messageId(first));
  } catch (error) {
    console.error(error);
...
async function openCompose(replyToMessage = null) {
  ...
  if (replyToMessage) {
    const replyRecipient = activeView === 'inbox'
      ? messageValue(replyToMessage, 'Sender', 'sender')
      : messageValue(replyToMessage, 'Recipient', 'recipient');
    const originalSubject = String(messageValue(replyToMessage, 'Subject', 'subject') || '').trim();
    const replyChoice = recipientChoices.find((choice) => choice.username.toLowerCase() === String(replyRecipient || '').trim().toLowerCase());
    if (replyChoice) selectedRecipients = [replyChoice];
    renderRecipientPicker();
...
async function sendMessage(event) {
  ...
  if (!recipients.length || !subject || !bodyText) return;

  try {
    await Promise.all(recipients.map((recipient) => postMessage({ recipient: recipient.username, recipientName: recipient.name, subject, body, attachments: selectedAttachments })));
    closeCompose();
...
function renderRecipientPicker() {
  ...
  const matches = recipientChoices.filter((choice) => !selectedRecipients.some((selected) => selected.username === choice.username) && (!query || choice.name.toLowerCase().includes(query)));
...
async function postMessage({ recipient, recipientName, subject, body, threadId: existingThreadId = '', attachments = [] }) {
  const appScriptUrl = window.GS_CONFIG ? window.GS_CONFIG.appScriptUrl : '';
...
messageViewer.addEventListener('click', (event) => {
  ...
  if (cancelButton) {
    ...
    if (replyForm) {
      ...
      const replyButton = messageViewer.querySelector(`[data-reply-message-id="${CSS.escape(replyForm.dataset.inlineReplyId)}"]`);
...
messageViewer.addEventListener('submit', async (event) => {
  ...
  const originalSender = String(messageValue(original, 'Sender', 'sender')).toLowerCase();
  const recipient = originalSender === currentUsername().toLowerCase()
    ? messageValue(original, 'Recipient', 'recipient')
    : messageValue(original, 'Sender', 'sender');
  const originalSubject = String(messageValue(original, 'Subject', 'subject') || '').trim();
  if (!bodyText || !recipient) return;

  try {
    await postMessage({
      recipient,
      recipientName: originalSender === currentUsername().toLowerCase()
        ? messageValue(original, 'Recipient Name', 'recipientName')
        : messageValue(original, 'Sender Name', 'senderName'),
